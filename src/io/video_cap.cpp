...
bool VideoCap::open(const char *url) {
  ...
  if (this->fmt_ctx != NULL) goto error;
  ...
  if (avformat_open_input(&(this->fmt_ctx), url, NULL, &(this->opts)) < 0) goto error;
  ...
  if (avformat_find_stream_info(this->fmt_ctx, NULL) < 0) goto error;
  ...
  if (!this->video_dec_ctx) goto error;

  // fill the codec context based on the values from the supplied codec parameters
  if (avcodec_parameters_to_context(this->video_dec_ctx, st->codecpar) < 0) goto error;
  ...
  if (avcodec_open2(this->video_dec_ctx, this->codec, &(this->opts)) < 0) goto error;
  ...
  if (!this->frame) goto error;

  if (this->video_stream_idx >= 0) valid = true;

error:

  if (!valid) this->release();
...
bool VideoCap::grab(void) {
  ...
  if (!this->fmt_ctx || !this->video_stream) return false;
  ...
  while (!valid) {
    ...
    if (got_frame) {
      ...
      // if no RTSP is used or no RTP timestamp <-> NTP walltime mapping is received, make timestamp from local system
      // time
      else {
...
bool VideoCap::retrieve(uint8_t **frame, int *step, int *width, int *height, int *cn, char *frame_type,
  ...
  if (!this->video_stream || !(this->frame->data[0])) return false;

  if (this->img_convert_ctx == NULL || this->picture.width != this->video_dec_ctx->width ||
      this->picture.height != this->video_dec_ctx->height || this->picture.data == NULL) {
    ...
    this->img_convert_ctx =
        sws_getCachedContext(this->img_convert_ctx, buffer_width, buffer_height, this->video_dec_ctx->pix_fmt,
                             buffer_width, buffer_height, AV_PIX_FMT_BGR24, SWS_BICUBIC, NULL, NULL, NULL);

    if (this->img_convert_ctx == NULL) return false;

    av_frame_unref(&(this->rgb_frame));
    this->rgb_frame.format = AV_PIX_FMT_BGR24;
    this->rgb_frame.width = buffer_width;
    this->rgb_frame.height = buffer_height;
    if (0 != av_frame_get_buffer(&(this->rgb_frame), 32)) return false;
  ...
  if (sd) {
    ...
    *num_mvs = sd->size / sizeof(*mvs);
    ...
    if (*num_mvs > 0) {
      ...
      if (!(*motion_vectors = (MVS_DTYPE *)malloc(*num_mvs * 10 * sizeof(MVS_DTYPE)))) return false;
...
bool VideoCap::read(uint8_t **frame, int *step, int *width, int *height, int *cn, char *frame_type,
                    MVS_DTYPE **motion_vectors, MVS_DTYPE *num_mvs, double *frame_timestamp) {
  bool ret = this->grab();
  if (ret) ret = this->retrieve(frame, step, width, height, cn, frame_type, motion_vectors, num_mvs, frame_timestamp);
  return ret;
}
...
bool VideoCap::check_format_rtsp(const char *format_names) {
  ...
  while ((format_name = strtok_r(buffer, ",", &buffer))) {
    if (strcmp(format_name, "rtsp") == 0) return true;
  }
